...
## Week 4 Challenges
# Challenge 0 - loop and concatenate
Using a `for` loop, display the output in console as shown below
    `EmployeeId::1`
    `EmployeeId::2`
    `EmployeeId::3`
    `EmployeeId::4`
...
Create a function which takes a state as input and looks through the data to output the cities that belong to that state. The data can be found in the file [cities.json](data/cities.json)
